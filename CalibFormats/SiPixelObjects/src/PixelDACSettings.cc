...
PixelDACSettings::PixelDACSettings(std::string filename):
  ...
  if (filename[filename.size()-1]=='t'){
	...
	while (!in.eof()){
		...
		std::shared_ptr<pos::PixelP1ROCDACSettings> tmp(new pos::PixelP1ROCDACSettings());
		tmp->read(in,rocid);
		dacsettings_.push_back(tmp); //may or may not be problematic
		in >> tag;
	...
	//ROCType_ = analog;
      ...
      //	    std::cout << "[PixelDACSettings::PixelDACSettings()] DAC setting ROC id:"<<rocid<<std::endl;
      		std::shared_ptr<PixelP0ROCDACSettings> tmp(new PixelP0ROCDACSettings());
      		tmp->read(in,rocid);
    ...
    while (!in.eof()){
      ...
      std::unique_ptr<PixelROCDACSettings> tmp;
...
bool PixelDACSettings::rocIsDisabled(const PixelDetectorConfig* detconfig, const PixelROCName rocname) const {
  std::cout << "HELLO " << rocname << std::endl;
