...
std::string pixDCDCCommand(FecAccess* fecAccess,
  ...
  try {
    ...
    if (turnOn + initPgood != 1) {
      ...
      if (turnOn)
	fecAccess->setPiaChannelDataReg(enableKey, invBits & fecAccess->getPiaChannelDataReg(pgoodKey));
      else
	fecAccess->setPiaChannelDataReg(enableKey, bits | fecAccess->getPiaChannelDataReg(pgoodKey));
